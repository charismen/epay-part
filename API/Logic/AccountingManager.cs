...
namespace Plexform.Base.Accounting
{
    public class AccountingManager : BaseClass
    {
        ...
        public async Task<AjaxResponse> SuspendCustomer(Accounting.Model.SuspendCustomerRequest input)
        {
            ...
            var contSubType = "ACT";
            var contResult = "Active";
            var updateBy = AbpSession.UserId.HasValue ? AbpSession.UserId.ToString() : "";
            //status = 2 | 2016 : Suspend
            //status = 4 | 2017 : Credit Suspend
            ...
            try
            {
                ...
                // bizEntityRes.LastUpdate = DateTime.Now;
                bizEntityRes.SyncLastUpd = DateTime.Now;
                bizEntityRes.LastSyncBy = updateBy;
                ...
                //    EmailType = 0,
                //    EmailAddress = bizEntityRes.Email,
                //    ExpiryDate = dateNow.AddDays(60).Date,
                //    Currency = "",
                //    BalanceDue = 0,
                //    PaymentAmt = 0,
                //    TransTotalAmt = 0,
                ...
                {
                    dateNow = dateNow.Date;
                    var DataPost = new ScheduleSuspendSP()
                    {
                        customerID = bizEntityRes.BizRegID,
                        updateBy = AbpSession.UserId.ToString(),
                        statusTo = contScheduleStatus,
                        statusCount = contStatus,
                        plannedStartDate = dateNow.ToString("yyyy-MM-dd"),
                    };
                    var dataSP = bizEntityLogic.StoredProcedure<ScheduleSuspendSP>("dbo.SuspendSchedule", DataPost);
                }
                //if (scheduleHdrRes != null && scheduleHdrRes.Count() > 0)
                //{
                //    using (TransactionScope scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        ...
        public async Task<List<DTO.Accounting.INVOICEITEM>> GetOpenInvoice(string id)
        {
            try
            {
                //var tempBizRegID = id;
                //var invoiceHDRLogic = new Accounting.Repo.INVOICEHDR(_env, _connectionString);
                //return invoiceHDRLogic.RetrieveAll(x => x.BizRegID == tempBizRegID && x.Status != 2);

                var tempBizRegID = id;
                var invoiceItemLogic = new Accounting.Repo.INVOICEITEM(_env, _connectionString);
                return invoiceItemLogic.RetrieveAll(x => x.BizRegID == tempBizRegID && x.Status != 2);
            }
            catch (Exception ex)
            {
        ...
        #region get pdf from invoice
        public async Task<dynamic> GetInvoiceList(GetInvoiceParameter input)
        {
            try
            {
                //var invoiceDTLLogic = new Accounting.Repo.INVOICEDTL(_env, _connectionString);
                var invoiceDTLLogic = new INVOICEITEM(_env, _connectionString);
                //var invoiceHDRLogic = new INVOICEHDR_OLD(_env, _connectionString);
                var sysPrefbLogic = new Core.General.Repo.SYSPREFB(_env, _connectionString);
                _serverExtAddress = _appConfiguration["AppExternal:ServerDomainAddress"];
                var tid = AbpSession.TenantId;

                string bizregid = input.bizregid;
                string transno = input.transNo;
                string type = input.type;
                string sorting = input.Sorting;
                string filter = !string.IsNullOrEmpty(input.Filter) ? input.Filter.Trim() : string.Empty;
                string status = input.status;
                //var interval = Convert.ToInt32(_OverdueInterval);
                int interval;
                var overdue = sysPrefbLogic.Retrieve(x => x.SYSKey == "OVERDUEINTERVAL" && x.BranchID == "PLX");
                if (overdue != null)
                {
                    interval = Convert.ToInt32(overdue.SYSValue);
                }
                else
                {
                    return false;
                ...
                #region Old LinQ
                //var data = invoiceDTLLogic
                //   .Select<DTO.Accounting.INVOICEDTL, 
                //   DTO.Accounting.INVOICEHDR,
                //   DTO.Core.Entity.BIZENTITY, 
                //   DTO.Core.Entity.BIZLOCATE, 
                //   DTO.Core.General.CODEMASTER,
                //   DTO.Core.General.CODEMASTER>((id, ih, be, bl, cm, cm1) => new
                //   {
                //       Status = cm1.CodeDesc,
                //       id.TransNo,
                //       Date = ih.PostDate,
                //       TotalInvoice = ih.TransAmt,
                //       DocumentType = cm.CodeDesc,
                //       id.BizRegID,
                //       id.BizLocID,
                //       SWTTANo = string.Format($"CASE WHEN len(id.bizregid) >8 THEN 'TBA' ELSE id.bizregid END"),
                //       BusinessName = be.CompanyName,
                //       ClinicName = bl.BranchName,
                //       DocumentNo = id.ItemCode,
                //       id.ItemType,
                //       InvoiceAmt = ih.TransSubTotal,
                //       DebitNoteAmt = ih.TransAmtRnd,
                //       CreditNoteAmt = ih.TransAmtSave,
                //       ManualPaymentAmt = ih.TransAmtOrg,
                //       id.PathRef,
                //       //PhysicalPath = "C:\\PDFFiles\\Invoice\\Success\\",
                //       ServerPath = _serverExtAddress,
                //   })
                //   .InnerJoin<DTO.Accounting.INVOICEHDR>(id => id.TransNo, ih => ih.TransNo)
                //   .InnerJoin<DTO.Core.Entity.BIZENTITY>(id => id.BizRegID, be => be.BizRegID)
                //   .InnerJoin<DTO.Core.Entity.BIZLOCATE>(id => id.BizLocID, bl => bl.BizLocID)
                //   .InnerJoin<DTO.Core.General.CODEMASTER>(id => id.ItemType, cm => cm.Code, cm => cm.CodeType == "INV")
                //   .InnerJoin<DTO.Core.General.CODEMASTER>(id => id.Status, cm1 => cm1.Code, cm1 => cm1.CodeType == "IVT")
                //   .Where<DTO.Accounting.INVOICEDTL, DTO.Accounting.INVOICEHDR, DTO.Core.Entity.BIZENTITY, DTO.Core.Entity.BIZLOCATE>((id, ih, be, bl) =>                       
                //       (!string.IsNullOrEmpty(bizregid) ? id.BizRegID == bizregid : false) &&
                //       (!string.IsNullOrEmpty(transno) ? id.TransNo == transno : false) &&
                //       (!string.IsNullOrEmpty(type) ? id.ItemType == type : false) &&
                //       (!string.IsNullOrEmpty(tempStatus) ? id.Status.Equals(tempStatus) : false) &&
                //       ((!string.IsNullOrEmpty(filter) ? id.BizRegID.Contains(filter) : false) ||
                //       (!string.IsNullOrEmpty(filter) ? be.CompanyName.Contains(filter) : false) ||
                //       (!string.IsNullOrEmpty(filter) ? bl.BranchName.Contains(filter) : false) ||
                //       (!string.IsNullOrEmpty(filter) ? id.TransNo.Contains(filter) : false)
                //       ))
                //   .GroupBy<DTO.Accounting.INVOICEDTL,
                //       DTO.Accounting.INVOICEHDR,
                //       DTO.Core.Entity.BIZENTITY,
                ...
                //           id.PathRef
                //       })
                //   .OrderBy<DTO.Accounting.INVOICEHDR>(ih => (!string.IsNullOrEmpty(sorting)? sorting : "ih.PostDate DESC"))
                //   .Fetch(input.SkipCount > 0 ? input.SkipCount : 0, input.MaxResultCount > 0 ? input.MaxResultCount : 0)
                //   .ExecuteList<Map.MapListPdfInvoice>();
                //.ToString();
                #endregion

                #region New Query
                //                SELECT cm1.codedesc AS Status, 
                //       id.BatchCode AS transno, 
                //       id.InvDate AS Date, 
                //       ih.TotalAmt1 AS TotalInvoice, 
                //       cm.codedesc AS DocumentType, 
                //       id.bizregid, 
                //       bl.bizlocid, 
                //       CASE
                //         WHEN Len(id.bizregid) > 8 THEN 'TBA'
                //         ELSE id.bizregid
                //       END AS SWTTANo,
                //       be.companyname AS BusinessName, 
                //       bl.branchname AS ClinicName, 
                //       id.BatchCode AS DocumentNo, 
                //       id.itemtype, 
                //       ih.TotalAmt2 AS InvoiceAmt, 
                //       ih.TotalAmt3 AS DebitNoteAmt, 
                //       ih.TotalAmt4 AS CreditNoteAmt, 
                //       ih.TotalAmt5 AS ManualPaymentAmt, 
                //       id.FilePath AS pathref, 
                //       NULL AS ServerPath
                //FROM INVOICEITEM id WITH(nolock)
                //INNER JOIN INVOICEHDR_OLD ih WITH(nolock) ON id.BatchCode = ih.BatchCode AND id.MonthCode = ih.MonthCode
                //INNER JOIN BIZLOCATE bl WITH(nolock) ON bl.BizLocID = id.BizRegID
                //INNER JOIN BIZENTITY be WITH(nolock) ON be.BizRegID = bl.BizRegID
                //INNER JOIN codemaster cm WITH(nolock) ON id.itemtype = cm.code AND cm.codetype = 'INV'
                //INNER JOIN codemaster cm1 WITH(nolock) ON id.status = cm1.code AND cm1.codetype = 'IVT'
                //WHERE ih.Status IN(1,2)
                //GROUP BY cm1.codedesc, 
                //          id.BatchCode, 
                //          id.InvDate, 
                //		  ih.TotalAmt1,
                //          ih.TotalAmt2, 
                ...
                //          ih.TotalAmt5, 
                //          id.FilePath
                //ORDER  BY id.InvDate DESC;
                #endregion

                var data = invoiceDTLLogic
                   .Select<DTO.Accounting.INVOICEITEM,
                   DTO.Questil.Payment.INVOICEHDR_CWMS,
                   DTO.Core.Entity.BIZENTITY,
                   DTO.Core.Entity.BIZLOCATE,
                   DTO.Core.General.CODEMASTER,
                   DTO.Core.General.CODEMASTER>((id, ih, be, bl, cm, cm1) => new
                   {
                       //Status = cm1.CodeDesc,                       
                       Status = string.Format($"CASE WHEN id.Status != 1 THEN cm1.CodeDesc WHEN (DATEDIFF(DAY, DATEADD(DAY, { interval }, id.ExpiryDate), GETDATE())) < { interval } THEN 'Open' ELSE 'Overdue' END"),
                       InvoiceNo = id.BatchCode,
                       InvoiceDate = id.InvDate,
                       Amount = string.Format($"ABS({ih.TotalAmt1})"),
                       DocType = cm.CodeDesc,
                       id.BizRegID,
                       bl.BizLocID,
                       SWTTANo = string.Format($"CASE WHEN len({id.BizRegID}) >8 THEN 'TBA' ELSE {id.BizRegID} END"),
                       BusinessName = be.CompanyName,
                       ClinicName = bl.BranchName,
                       DocumentNo = id.BatchCode,
                       id.ItemType,
                       InvoiceAmt = ih.TotalAmt2,
                       DebitNoteAmt = ih.TotalAmt3,
                       CreditNoteAmt = ih.TotalAmt4,
                       ManualPaymentAmt = ih.TotalAmt5,
                       PathRef = id.FilePath,
                       //PhysicalPath = "C:\\PDFFiles\\Invoice\\Success\\",
                       ServerPath = _serverExtAddress
                   })
                 ...
                 (
                     invoiceHDRLogic
                     .Select<DTO.Accounting.INVOICEHDR_OLD, DTO.Accounting.INVOICEITEM>((i, id) => new
                     {
                         InvoiceAging = $"DATEDIFF(DAY, {id.ExpiryDate}, GETDATE())"
                     })
                     .Where<DTO.Accounting.INVOICEHDR_OLD, DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZENTITY>((i, id, be) =>
                            i.Flag == 1 && i.Inuse == 0 && id.BizRegID == be.BizRegID)
                     .ToString()
                 )*/
                   .InnerJoin<DTO.Questil.Payment.INVOICEHDR_CWMS>(id => id.BatchCode, ih => ih.BatchCode, (id, ih) => id.MonthCode == ih.MonthCode)
                   .InnerJoin<DTO.Core.Entity.BIZENTITY>(id => id.BizRegID, be => be.BizRegID)
                   .InnerJoin<DTO.Core.Entity.BIZLOCATE>(be => be.BizRegID, bl => bl.BizRegID)
                   .InnerJoin<DTO.Core.General.CODEMASTER>(id => id.ItemType, cm => cm.Code, cm => cm.CodeType == "INV")
                   .InnerJoin<DTO.Accounting.INVOICEITEM, DTO.Core.General.CODEMASTER>((id) => id.Status, cm1 => cm1.Code, cm1 => cm1.CodeType == "IVT")
                   .Where<DTO.Accounting.INVOICEITEM, DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Core.Entity.BIZENTITY, DTO.Core.Entity.BIZLOCATE>((id, ih, be, bl) =>
                   id.Flag == 1 && id.TID == tid && id.Status != 3 && id.Status != 8 &&
                       (!string.IsNullOrEmpty(bizregid) ? id.BizRegID == bizregid : false) &&
                       (!string.IsNullOrEmpty(transno) ? id.BatchCode == transno : false) &&
                       (!string.IsNullOrEmpty(type) ? id.ItemType == type : false) &&
                       (!string.IsNullOrEmpty(status) ? (status == "2" ? id.Status.Equals(status) : id.Status != 2) : false) &&
                       //(!string.IsNullOrEmpty(status) ? ih.Status.Equals(status) : false) &&
                       ((!string.IsNullOrEmpty(filter) ? id.BizRegID.Contains(filter) : false) ||
                       (!string.IsNullOrEmpty(filter) ? be.CompanyName.Contains(filter) : false) ||
                       (!string.IsNullOrEmpty(filter) ? bl.BranchName.Contains(filter) : false) ||
                       (!string.IsNullOrEmpty(filter) ? id.BatchCode.Contains(filter) : false)
                       ))
                   .GroupBy<DTO.Accounting.INVOICEITEM,
                       DTO.Questil.Payment.INVOICEHDR_CWMS,
                       DTO.Core.Entity.BIZENTITY,
                       DTO.Core.Entity.BIZLOCATE,
                       DTO.Core.General.CODEMASTER,
                   ...
                   .OrderBy<DTO.Accounting.INVOICEITEM>(id => (!string.IsNullOrEmpty(sorting) ? sorting : "id.InvDate  DESC"))
                   .Fetch(input.SkipCount > 0 ? input.SkipCount : 0, input.MaxResultCount > 0 ? input.MaxResultCount : 0)
                   .ExecuteList<Map.MapListPdfInvoice>();
                //.ToString();
        ...
        public async Task<dynamic> GetInvoiceHeaderNew(string transno)
        {
            try
            {
                //SELECT TOP 100 th.TransNo[MerchantOrderNo], ih.createdate, ISNULL(tt.BillNo, '')[FPXTransID], th.TransDate[TransactionDate], th.CustPrivilege[BankID], cm.CodeDesc[BankName], ih.TransNo[InvoiceNo], ih.PostDate[InvoiceDate], ih.Status, cms.CodeDesc[StatusDesc], th.TransAmt[PaidAmount], ih.TransAmt[TotalInvoiceAmt], ih.TransSubTotal[InvoiceAmt], th.TransAmtRnd[DebitNoteAmt], th.TransAmtSave[CreditNoteAmt], th.TransAmtOrg[ManualAmt]
                //FROM INVOICEHDR ih
                //INNER JOIN INVOICEDTL id ON id.TransNo = ih.TransNo AND id.ItemType = 0
                //INNER JOIN BIZLOCATE bl ON bl.BizLocID = ih.BizLocID
                //INNER JOIN BIZENTITY bz ON bz.BizRegID = bl.BizRegID
                //INNER JOIN CODEMASTER cms ON cms.Code = ih.Status AND cms.CodeType = 'IVT'
                //LEFT JOIN TRANSDTL td ON td.SerialNo = id.TransNo
                //LEFT JOIN TRANSHDR th ON th.TransNo = td.TransNo AND th.Status = 2
                //LEFT jOIN TRANSTENDER tt ON tt.TransNo = th.TransNo AND th.Status = 2
                //LEFT JOIN CODEMASTER cm ON cm.Code = th.CustPrivilege AND cm.CodeType IN('BNB')
                //--WHERE ISNULL(th.TransNo, '') <> ''
                //GROUP BY th.TransNo, ih.createdate, tt.BillNo, th.TransDate, th.CustPrivilege, cm.CodeDesc, ih.TransNo, ih.PostDate, ih.Status, cms.CodeDesc, th.TransAmt, ih.TransAmt, ih.TransSubTotal, th.TransAmtRnd, th.TransAmtSave, th.TransAmtOrg, ih.CreateDate
                //ORDER BY ih.CreateDate DESC
                var invoiceHdrLogic = new INVOICEHDR(_env, _connectionString);
                var sysPrefbLogic = new SYSPREFB(_env, _connectionString);
                ...
                var data = invoiceHdrLogic
                   ...
                   DTO.Accounting.TRANSTENDER,
                   DTO.Core.General.CODEMASTER,
                   DTO.Core.General.CODEMASTER>((ih, id, be, th, tt, cm, cm2) => new
                   {
                       be.BizRegID,
                       be.CompanyName,
                       MerchantOrderNo = th.TransNo,
                       ih.CreateDate,
                       FPXTransID = string.Format($"ISNULL({tt.BillNo}, '')"),
                       TransactionDate = th.TransDate,
                       BankID = th.CustPrivilege,
                       BankName = cm.CodeDesc,
                       InvoiceNo = ih.TransNo,
                       InvoiceDate = ih.PostDate,
                       ih.Status,
                       StatusDesc = cm2.CodeDesc,
                       //StatusDesc = string.Format($"CASE WHEN ih.Status = 2 THEN cm2.CodeDesc WHEN (DATEDIFF(DAY, DATEADD(DAY, { interval }, id.ExpiryDate), GETDATE())) < { interval } THEN 'OPEN' ELSE 'OVERDUE' END"),
                       PaidAmount = th.TransAmt,
                       TotalInvoiceAmt = string.Format($"ABS({ih.TransAmt})"),
                       InvoiceAmt = ih.TransSubTotal,
                       DebitNoteAmt = th.TransAmtRnd,
                       CreditNoteAmt = th.TransAmtSave,
                       ManualAmt = th.TransAmtOrg,
                       StatusColor = cm2.CodeVal1
                   })
                   .InnerJoin<DTO.Accounting.INVOICEDTL>(ih => ih.TransNo, id => id.TransNo, id => id.ItemType == "0")
                   .InnerJoin<DTO.Core.Entity.BIZLOCATE>(ih => ih.BizLocID, bl => bl.BizLocID)
                   .InnerJoin<DTO.Core.Entity.BIZLOCATE, DTO.Core.Entity.BIZENTITY>(bl => bl.BizRegID, be => be.BizRegID)
                   .InnerJoin<DTO.Core.General.CODEMASTER>(ih => ih.Status, cm2 => cm2.Code, cm2 => cm2.CodeType == "IVT")
                   .LeftJoin<DTO.Accounting.INVOICEDTL, DTO.Accounting.TRANSDTL>(id => id.TransNo, td => td.SerialNo)
                   .LeftJoin<DTO.Accounting.TRANSDTL, DTO.Accounting.TRANSHDR>(td => td.TransNo, th => th.TransNo, th => th.Status == 2)
                   .LeftJoin<DTO.Accounting.TRANSHDR, DTO.Accounting.TRANSTENDER>(th => th.TransNo, tt => tt.TransNo)
                   .LeftJoin<DTO.Accounting.TRANSHDR, DTO.Core.General.CODEMASTER>(th => th.CustPrivilege, cm => cm.Code, cm => cm.CodeType == "BNB")
                   .Where(ih => (!string.IsNullOrEmpty(transno) ? ih.TransNo == transno : false))
                   .GroupBy<DTO.Accounting.INVOICEHDR,
                       DTO.Accounting.INVOICEDTL,
                       DTO.Core.Entity.BIZENTITY,
                       DTO.Accounting.TRANSHDR,
        ...
        public async Task<dynamic> GetInvoiceHeader(string transno)
        {
            try
            {
                #region Query
                //SELECT be.BizRegID, 
                //  be.CompanyName, 
                //  th.TransNo AS MerchantOrderNo, 
                //  ih.CreateDate, 
                //  ISNULL(tt.BillNo, '') AS FPXTransID,
                //        th.TransDate AS TransactionDate, 
                //  th.CustPrivilege AS BankID, 
                //  cm.CodeDesc AS BankName, 
                //  ih.BatchCode AS InvoiceNo, 
                //  id.InvDate AS InvoiceDate, 
                //  ih.Status, 
                //  cm2.CodeDesc AS StatusDesc, 
                //  th.TransAmt AS PaidAmount, 
                //  ih.TotalAmt1 AS TotalInvoiceAmt, 
                //  ih.TotalAmt2 AS InvoiceAmt, 
                //  ih.TotalAmt3 AS DebitNoteAmt, 
                //  ih.TotalAmt4 AS CreditNoteAmt, 
                //  ih.TotalAmt5 AS ManualAmt
                //FROM INVOICEHDR_OLD ih WITH(NOLOCK)
                //INNER JOIN INVOICEITEM id WITH(NOLOCK) ON ih.BatchCode = id.BatchCode AND id.ItemType = '0'
                //INNER JOIN BIZLOCATE bl WITH(NOLOCK) ON id.BizRegID = bl.BizLocID
                //INNER JOIN BIZENTITY be WITH(NOLOCK) ON bl.BizRegID = be.BizRegID
                //INNER JOIN CODEMASTER cm2 WITH(NOLOCK) ON ih.Status = cm2.Code AND cm2.CodeType = 'IVT'
                //LEFT JOIN TRANSDTL td WITH(NOLOCK) ON id.BatchCode = td.SerialNo
                //LEFT JOIN TRANSHDR th WITH(NOLOCK) ON td.TransNo = th.TransNo AND th.Status = 2
                //LEFT JOIN TRANSTENDER tt WITH(NOLOCK) ON th.TransNo = tt.TransNo
                //LEFT JOIN CODEMASTER cm WITH(NOLOCK) ON th.CustPrivilege = cm.Code AND cm.CodeType IN('BNB', 'BNC')
                //WHERE ih.BatchCode = '7000069690'
                //GROUP BY    be.BizRegID, 
                //   be.CompanyName, 
                //   th.TransNo, 
                //   ih.CreateDate, 
                //   tt.BillNo, 
                ...
                //var invoiceHdrLogic = new INVOICEHDR_OLD(_env, _connectionString);
                var codemasterLogic = new CODEMASTER(_env, _connectionString);
                var sysPrefbLogic = new Core.General.Repo.SYSPREFB(_env, _connectionString);
                string[] code = { "BNB", "BNC" };
                int interval;
                ...
                var openData = codemasterLogic.Retrieve(x => x.CodeType == "IVT" && x.Code == "0");
                var overdueData = codemasterLogic.Retrieve(x => x.CodeType == "IVT" && x.Code == "4");
                string openStatus = (openData != null ? openData.CodeDesc : "Open");
                string openColor = (openData != null ? openData.CodeVal1 : "#000000");
                string overdueStatus = (overdueData != null ? overdueData.CodeDesc : "Overdue");
                string overdueColor = (overdueData != null ? overdueData.CodeVal1 : "#000000");

                var data = _invoiceHDRRepo
                   .Select<DTO.Questil.Payment.INVOICEHDR_CWMS,
                   DTO.Accounting.INVOICEITEM,
                   DTO.Core.Entity.BIZENTITY,
                   DTO.Accounting.TRANSHDR,
                   DTO.Accounting.TRANSTENDER,
                   DTO.Core.General.CODEMASTER,
                   DTO.Core.General.CODEMASTER>((ih, id, be, th, tt, cm, cm2) => new
                   {
                       be.BizRegID,
                       be.CompanyName,
                       MerchantOrderNo = string.Format($"ISNULL({th.TransNo}, '-')"),
                       ih.CreateDate,
                       FPXTransID = string.Format($"ISNULL({tt.BillNo}, '-')"),
                       TransactionDate = th.TransDate,
                       BankID = th.CustPrivilege,
                       BankName = string.Format($"ISNULL({cm.CodeDesc}, '-')"),
                       InvoiceNo = ih.BatchCode,
                       InvoiceDate = string.Format($"convert(varchar, {id.InvDate}, 103)"),
                       ih.Status,
                       StatusDesc = string.Format("CASE WHEN ih.Status = 2 THEN cm2.CodeDesc WHEN (DATEDIFF(DAY, DATEADD(DAY, {0}, id.ExpiryDate), GETDATE())) < {0} THEN {1} ELSE {2} END", interval, openStatus, overdueStatus),
                       //StatusDesc = cm2.CodeDesc,
                       PaidAmount = th.TransAmt,
                       TotalInvoiceAmt = string.Format($"ABS({ih.TotalAmt1})"),
                       InvoiceAmt = ih.TotalAmt2,
                       DebitNoteAmt = ih.TotalAmt3,
                       CreditNoteAmt = ih.TotalAmt4,
                       ManualAmt = ih.TotalAmt5,
                       StatusColor = string.Format("CASE WHEN ih.Status = 2 THEN cm2.CodeVal1 WHEN (DATEDIFF(DAY, DATEADD(DAY, {0}, id.ExpiryDate), GETDATE())) < {0} THEN {1} ELSE {2} END", interval, openColor, overdueColor)
                   })
                   .InnerJoin<DTO.Accounting.INVOICEITEM>(ih => ih.BatchCode, id => id.BatchCode, id => id.ItemType == "0")
                   .InnerJoin<DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZLOCATE>(id => id.BizRegID, bl => bl.BizLocID)
                   .InnerJoin<DTO.Core.Entity.BIZLOCATE, DTO.Core.Entity.BIZENTITY>(bl => bl.BizRegID, be => be.BizRegID)
                   .InnerJoin<DTO.Core.General.CODEMASTER>(ih => ih.Status, cm2 => cm2.Code, cm2 => cm2.CodeType == "IVT")
                   .LeftJoin<DTO.Accounting.INVOICEITEM, DTO.Accounting.TRANSDTL>(id => id.BatchCode, td => td.ItemCode)
                   .LeftJoin<DTO.Accounting.TRANSDTL, DTO.Accounting.TRANSHDR>(td => td.TransNo, th => th.TransNo, th => th.Status == 2)
                   .LeftJoin<DTO.Accounting.TRANSHDR, DTO.Accounting.TRANSTENDER>(th => th.TransNo, tt => tt.TransNo)
                   .LeftJoin<DTO.Accounting.TRANSHDR, DTO.Core.General.CODEMASTER>(th => th.CustPrivilege, cm => cm.Code, cm => code.Equals(cm.CodeType))
                   .Where(ih => (!string.IsNullOrEmpty(transno) ? ih.BatchCode == transno : false))
                   .GroupBy<DTO.Questil.Payment.INVOICEHDR_CWMS,
                       DTO.Accounting.INVOICEITEM,
                       DTO.Core.Entity.BIZENTITY,
                       DTO.Accounting.TRANSHDR,
                       DTO.Accounting.TRANSTENDER,
        ...
        public async Task<dynamic> GetInvoiceDetails(string bizregid, string transno)
        {
            try
            {
                #region Query
                //SELECT iix.ItemType, cmx.CodeDesc, iix.InvDate[DocumentDate], iix.ItemCode[DocNo], iix.Amount1[DebitValue], iix.Amount2[CreditValue], iix.Currency, iix.FilePath
                //FROM INVOICEITEM iix
                //INNER JOIN CODEMASTER cmx ON cmx.CodeType = 'INV' AND cmx.Code = iix.ItemType
                //WHERE iix.BatchCode = '7000013378' AND iix.BizRegID = '203287'
                //UNION
                //SELECT ti.ItemType, cm.CodeDesc, ti.CNDNDate[DocumentDate], ti.ItemCode[DocNo], ti.Amount1[DebitValue], ti.Amount2[CreditValue], ti.Currency, ti.FilePath
                //FROM TRANSCDITEM ti
                //INNER JOIN INVOICEHDR_OLD ih ON ih.BatchCode = ti.BatchCode AND ih.MonthCode = ti.MonthCode
                //INNER JOIN INVOICEITEM ii ON ii.BatchCode = ih.BatchCode AND ii.BizRegID = ti.BizRegID
                //INNER JOIN CODEMASTER cm ON cm.CodeType = 'INV' AND cm.Code = ti.ItemType
                //WHERE ti.BatchCode = '7000013378' AND ti.BizRegID = '203287'
                //GROUP BY ti.ItemType, cm.CodeDesc, ti.CNDNDate, ti.ItemCode, ti.ItemCode, ti.Amount1, ti.Amount2, ti.Currency, ti.FilePath
                #endregion

                var invoiceItemLogic = new Repo.INVOICEITEM(_env, _connectionString);
                var transcdItemLogic = new Repo.TRANSCDITEM(_env, _connectionString);

                _serverExtAddress = _appConfiguration["AppExternal:ServerDomainAddress"];
                string[] code = { "BNB", "BNC" };

                var data = invoiceItemLogic
                   .Select<DTO.Accounting.INVOICEITEM,
                   DTO.Core.General.CODEMASTER>((iix, cmx) => new
                   {
                       iix.ItemType,
                       DocumentNo = iix.ItemCode,
                       DocumentType = cmx.CodeDesc,
                       DocumentDate = iix.InvDate,
                       //DebitValue = iix.Amount1,
                       //CreditValue = iix.Amount2,
                       Amount = string.Format($"CASE WHEN {iix.Amount1} <> 0 THEN {iix.Amount1} ELSE {iix.Amount2} END "),
                       iix.Currency,
                       PathRef = iix.FilePath,
                       ServerPath = _serverExtAddress
                   })
                   .InnerJoin<DTO.Core.General.CODEMASTER>(iix => iix.ItemType, cmx => cmx.Code, cmx => cmx.CodeType == "INV")
                   .Where(iix => (!string.IsNullOrEmpty(bizregid) ? iix.BizRegID == bizregid : false) &&
                            (!string.IsNullOrEmpty(transno) ? iix.BatchCode == transno : false))
                   .Union(
                        transcdItemLogic
                        .Select<DTO.Accounting.TRANSCDITEM,
                           DTO.Core.General.CODEMASTER>((ti, cm) => new
                           {
                               ti.ItemType,
                               DocumentNo = ti.ItemCode,
                               DocumentType = cm.CodeDesc,
                               DocumentDate = ti.CNDNDate,
                               //DebitValue = ti.Amount1,
                               //CreditValue = ti.Amount2,
                               Amount = string.Format($"CASE WHEN {ti.Amount1} <> 0 THEN {ti.Amount1} ELSE {ti.Amount2} END "),
                               ti.Currency,
                               PathRef = ti.FilePath,
                               ServerPath = _serverExtAddress
                           })
                       .InnerJoin<DTO.Questil.Payment.INVOICEHDR_CWMS>(ti => ti.BatchCode, ih => ih.BatchCode, (ti, ih) => ti.MonthCode == ih.MonthCode)
                       .InnerJoin<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>(ih => ih.BatchCode, ii => ii.BatchCode)
                       .InnerJoin<DTO.Core.General.CODEMASTER>(ti => ti.ItemType, cm => cm.Code, cm => cm.CodeType == "INV")
                       .Where<DTO.Accounting.TRANSCDITEM,
                           DTO.Accounting.INVOICEITEM>((ti, ii) => (ii.BizRegID == ti.BizRegID) && (!string.IsNullOrEmpty(bizregid) ? ti.BizRegID == bizregid : false) &&
                            (!string.IsNullOrEmpty(transno) ? ti.BatchCode == transno : false))
                       .GroupBy<DTO.Accounting.TRANSCDITEM,
                           DTO.Core.General.CODEMASTER>((ti, cm) => new
                           {
                               ti.ItemType,
                               cm.CodeDesc,
                   ...
                   .ExecuteList<Map.MapListInvoiceDetails>();
                //.ToString();
        ...
        #region Get Invoice List Customer
        public async Task<dynamic> GetInvoiceQ(Model.InvoiceFilter input)
        {
            try
            {
                var invoiceQSumLogic = new BaseRepository<DTO.Questil.Payment.INVOICEHDR_CWMS>(_env, _connectionString);
                //  var invoiceQLogic = new INVOICEHDR_CWMS(_env, _connectionString);//INVOICEHDR_OLD(_env, _connectionString);
                // var invoiceQSumLogic = new INVOICEHDR_CWMS(_env, _connectionString);
                var sysPrefbLogic = new SYSPREFB(_env, _connectionString);
                string tempBizRegID;
                string sorting = input.Sorting;
                string filter = input.Filter;
                var DateNow = DateTime.Now;
                //string startDate = Convert.ToDateTime(input.dateStart).Date.ToString("yyyy-MM-dd");
                //string endDate = Convert.ToDateTime(input.dateEnd).Date.ToString("yyyy-MM-dd");
                var startDate = Convert.ToDateTime(input.dateStart).Date;
                var endDate = Convert.ToDateTime(input.dateEnd).Date.AddDays(1);
                var tempInvoiceNo = input.invoiceNo;
                tempBizRegID = input.customerCode;
                int? status = input.status;
                //var interval = Convert.ToInt32(_OverdueInterval);
                //DateTime tempDateStart = Convert.ToDateTime(input.dateStart);
                //DateTime tempDateEnd = Convert.ToDateTime(input.dateEnd);
                _serverExtAddress = _appConfiguration["AppExternal:ServerDomainAddress"];
                ...
                #region "Sync Invoice status with SAP Credit Status"
                try
                {
                    if (!string.IsNullOrEmpty(tempBizRegID))
                    {
                        //check customer credit status from SAP API
                        var custList = new List<string>() { tempBizRegID };
                        var resCheck = await _sapManager.CheckCustomerCredit(new List<INT.SAP.KA.Model.CheckCustomerCreditRequest>() { new INT.SAP.KA.Model.CheckCustomerCreditRequest() { CustomerCode = tempBizRegID } });
                        if (resCheck != null)
                        {
                            if (resCheck.CustomerCreditList != null)
                            {
                                if (resCheck.CustomerCreditList.Count() > 0)
                                {
                                    Int32 tempInvStatus = 1;
                                    Int32 tempCustStatus = 1;
                                    //Accounting.Repo.INVOICEHDR_OLD invHdrLogic = new Accounting.Repo.INVOICEHDR_OLD(_env, _connectionString);
                                    Accounting.Repo.INVOICEITEM invItemLogic = new Accounting.Repo.INVOICEITEM(_env, _connectionString);
                                    ...
                                    if (resCheck.CreditResponse != null)
                                    {
                                        if (resCheck.CreditResponse.Count() > 0)
                                        {
                                            foreach (var index in resCheck.CreditResponse)
                                            {
                                                if (Convert.ToInt32(index.Invoiceageing) >= interval)
                                                {
                                                    tempInvStatus = 4; //Invoice overdue status
                                                }
                                                //else if (Convert.ToInt32(index.Invoiceageing) >= 85 && Convert.ToInt32(index.Invoiceageing) <= 89) //remark sek, jare bos e gak perlu nearly overdue
                                                //{
                                                //    tempInvStatus = 3; //Invoice nearly overdue status
                                                //}
                                                ...
                                                var contInvoiceNo = index.InvoiceNo;
                                                var invItemRes = invItemLogic.Retrieve(x => x.BatchCode == contInvoiceNo && x.BizRegID == tempBizRegID);
                                                if (invItemRes != null)
                                                {
                                                    var tempMonthCode = invItemRes.MonthCode;
                                                    var tempBatchcode = invItemRes.BatchCode;
                                                    var invHdrRes = _invoiceHDRRepo.Retrieve(x => x.BatchCode == tempBatchcode && x.MonthCode == tempMonthCode);
                                                    if (invHdrRes != null)
                                                    {
                                                        if (invHdrRes.Status != 2)
                                                        {
                                                            System.DateTime dueDate = DateTime.ParseExact(index.InvoiceDueDate, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture);

                                                            invItemRes.Status = Convert.ToByte(tempInvStatus);
                                                            invItemRes.ExpiryDate = dueDate;
                                                            invItemRes.LastUpdate = DateTime.Now;
                                                            invItemRes.SyncLastUpd = DateTime.Now;
                                                            invItemRes.Inuse = 0;
                                                            invHdrRes.Status = Convert.ToByte(tempInvStatus);
                                                            ...
                                                            await invItemLogic.Update(invItemRes);
                                                            await _invoiceHDRRepo.Update(invHdrRes);
                                                        }
                                                    }
                                                }
                                            }

                                            #region "Update Customer Status base on Credit Status"
                                            //Remark sek, daripada deadlock
                                            //if (resCheck.CreditResponse.Any(x => Convert.ToInt32(x.Invoiceageing) >= 90))
                                            //{
                                            //    tempCustStatus = 4;
                                            //}
                ...
                {
                    string exxMsg = exx.Message.ToString();
                }
                #endregion
                //var orderBy = "CASE WHEN InvoiceStatus < 85 THEN 2 WHEN (InvoiceStatus >= 85 AND InvoiceStatus <= 89) THEN 1 ELSE 3 END";                

                var data = _invoiceHDRRepo
                    .Select<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZENTITY, DTO.Accounting.TRANSHDR, DTO.Core.General.CODEMASTER, DTO.Core.General.CODEMASTER>((ih, id, be, th, cm, cm1) => new
                    {
                        //InvoiceStatus = string.Format($"CASE WHEN InvoiceStatus < { interval } THEN 'Open' ELSE 'Overdue' END"),
                        //InvoiceStatus = string.Format($"CASE WHEN GETDATE() < id.ExpiryDate THEN 'Open' ELSE 'Overdue' END"),
                        InvoiceStatus = string.Format($"case when id.status = 1 then (case when id.inuse = 0 then (case when id.expirydate < GETDATE() then 'Overdue' when id.invdate < id.expirydate then 'Open' else 'Open' end) else 'In Progress' end) else cm1.CodeDesc end"),
                        InvoiceNo = ih.BatchCode,
                        SWTTANo = id.BizRegID,
                        Company = be.CompanyName,
                        issuedDate = id.InvDate,
                        DueDate = id.ExpiryDate,
                        FPXId = th.BillNo,
                        InvoiceAging = string.Format($"CASE WHEN InvoiceAging <= 0 THEN '0' ELSE InvoiceAging END"),
                        //InvoiceAging = string.Format($"CASE WHEN InvoiceAging <= 0 THEN '0' ELSE InvoiceAging END"),
                        Amount = string.Format($"ABS(ih.TotalAmt1)"),
                        Bank = cm.CodeDesc,
                        Status = cm1.CodeDesc,
                    ...
                    (
                        invoiceQSumLogic
                         .Select<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ix, idx) => new
                         {
                             InvoiceStatus = $"DATEDIFF(DAY, DATEADD(DAY, { interval }, { idx.ExpiryDate }), GETDATE())"
                         })
                        .InnerJoin<DTO.Accounting.INVOICEITEM>(ix => ix.BatchCode, idx => idx.BatchCode, (ix, idx) => ix.MonthCode == idx.MonthCode)
                        .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, ix, idx) =>
                              ix.Flag == 1 /*&& ix.Inuse == 0*/ && idx.BizRegID == tempBizRegID && idx.BatchCode == ih.BatchCode)
                        .ToString()
                    )
                    //count Invoice Aging
                    .OuterApply
                    (
                        invoiceQSumLogic
                        .Select<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((iy, idy) => new
                        {
                            InvoiceAging = $"DATEDIFF(DAY, { idy.ExpiryDate }, GETDATE())"
                        })
                        .InnerJoin<DTO.Accounting.INVOICEITEM>(iy => iy.BatchCode, idy => idy.BatchCode, (iy, idy) => iy.MonthCode == idy.MonthCode)
                        .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, iy, idy) =>
                               iy.Flag == 1 /*&& iy.Inuse == 0*/ && idy.BizRegID == tempBizRegID && idy.BatchCode == ih.BatchCode)
                        .ToString()
                    )
                    .InnerJoin<DTO.Accounting.INVOICEITEM>(ih => ih.BatchCode, id => id.BatchCode, (ih, id) => ih.MonthCode == id.MonthCode)
                    .LeftJoin<DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZENTITY>(id => id.BizRegID, be => be.BizRegID)
                    .LeftJoin<DTO.Accounting.TRANSDTL>(ih => ih.BatchCode, td => td.SerialNo)
                    .LeftJoin<DTO.Accounting.TRANSDTL, DTO.Accounting.TRANSHDR>(td => td.TransNo, th => th.TransNo)
                    .LeftJoin<DTO.Accounting.TRANSDTL, DTO.Accounting.TRANSTENDER>(td => td.BillNo, tt => tt.BillNo)
                    .LeftJoin<DTO.Accounting.INVOICEITEM, DTO.Core.General.CODEMASTER>(id => id.Status, cm1 => cm1.Code, cm1 => cm1.CodeType == "IVT")
                    .LeftJoin<DTO.Accounting.INVOICEITEM, DTO.Core.General.CODEMASTER>(id => id.ItemType, cm => cm.Code, cm => cm.CodeType == "BNB")
                    .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, id) =>
                           ih.Flag == 1 && id.Status != 3 && id.Status != 8// && ih.Inuse == 0
                           && (status != -1 ? (status == 2 ? id.Status == 2 : id.Status != 2) : false)
                           && id.BizRegID == tempBizRegID &&
                           //((input.dateStart != null && input.dateEnd != null) ? tempDateStart <= id.InvDate && tempDateEnd >= id.InvDate : false) &&
                           (input.dateStart != null ? startDate <= id.InvDate : false) &&
                           (input.dateEnd != null ? endDate >= id.InvDate : false) &&
                           //(input.dateStart != null ? tempDateStart <= id.InvDate : false) &&
                           //(input.dateEnd != null ? tempDateEnd >= id.InvDate : false) &&
                           (!string.IsNullOrEmpty(tempInvoiceNo) ? ih.BatchCode == tempInvoiceNo : false) &&
                           (!string.IsNullOrEmpty(filter) ? ih.BatchCode.Contains(filter) : false))
                    .GroupBy<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZENTITY, DTO.Accounting.TRANSHDR, DTO.Core.General.CODEMASTER, DTO.Core.General.CODEMASTER>((ih, id, be, th, cm, cm1) => new
                    {
                        //InvoiceStatus = false,
                        id.Status,
                        id.Inuse,
                   ...
                   .OrderBy<DTO.Accounting.INVOICEITEM>(id => (!string.IsNullOrEmpty(sorting) ? sorting : $"id.ExpiryDate"))
                    .Fetch(input.SkipCount > 0 ? input.SkipCount : 0, input.MaxResultCount > 0 ? input.MaxResultCount : 0)
                    .ExecuteList<Map.MapInvoiceQList>();
                //.ToString();
        ...
        public async Task<dynamic> CheckPendingPaymentStatus(string transNo)
        {
            try
            {
                string tempTransNo = transNo;
                var transHDRLogic = new Repo.TRANSHDR(_env, _connectionString);
                var data = transHDRLogic
                    .Select<DTO.Accounting.TRANSHDR,
                            DTO.Accounting.TRANSDTL,
                            DTO.Accounting.TRANSFPX,
                            DTO.Questil.Payment.INVOICEHDR_CWMS,
                            DTO.Accounting.INVOICEITEM,
                            DTO.Accounting.TRANSTENDER>
                    ((TH, TD, TF, IH, II, TT) => new
                    {
                        TH.TransNo,
                        TH.BillNo,
                        II.BizRegID,
                        ...
                        TH.TransStartTime,
                        PaidAmount = TH.TransAmt,
                        InvoiceAmount = IH.TotalAmt1,
                        TH.CustPrivilege,
                        PaymentStatus = TH.Status,
                        InvoiceStatus = IH.Status,
                        TF.RefInfo,
                        TF.CCrefInfo
                    })
                    .InnerJoin<DTO.Accounting.TRANSDTL>(TH => TH.TransNo, TD => TD.TransNo)
                    .InnerJoin<DTO.Accounting.TRANSFPX>(TH => TH.TransNo, TF => TF.TransNo)
                    .InnerJoin<DTO.Accounting.TRANSDTL, DTO.Questil.Payment.INVOICEHDR_CWMS>(TD => TD.ItemCode, IH => IH.BatchCode)
                    .InnerJoin<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>(IH => IH.BatchCode, II => II.BatchCode, (IH, II) => IH.MonthCode == II.MonthCode)
                    .LeftJoin<DTO.Accounting.TRANSTENDER>(TH => TH.TransNo, TT => TT.TransNo)
                    .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.TRANSHDR>((IH, TH) => IH.Status != 2 &&
                    (!string.IsNullOrEmpty(tempTransNo) ? TH.TransNo == tempTransNo : false))
                    .GroupBy<DTO.Accounting.TRANSHDR,
                            DTO.Accounting.TRANSDTL,
                            DTO.Accounting.TRANSFPX,
                            DTO.Questil.Payment.INVOICEHDR_CWMS,
                    ...
                    .OrderByDesc<DTO.Accounting.TRANSHDR>(TH => TH.TransDate)
                    .ExecuteList<Map.MapListPendingPaymentStatus>();
                //.ToString();
        ...
        #region total overdue
        public async Task<dynamic> GetTotalInvoiceQ(string BizRegID)
        {
            try
            {
                //   var invoiceQSumLogic = new Repo.INVOICEHDR_OLD(_env, _connectionString);
                var interval = Convert.ToInt32(_appConfiguration["InvoiceConfig:OverdueInterval"] != null ? _appConfiguration["InvoiceConfig:OverdueInterval"] : "90");

                var tempBizRegID = BizRegID;
                var dataOver = _invoiceHDRRepo
                  .Count(i => new
                  {
                      TotalOverdue = i.Status
                  })
                  .InnerJoin<DTO.Accounting.INVOICEITEM>(i => i.BatchCode, id => id.BatchCode, (i, id) => i.MonthCode == id.MonthCode)
                  .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((i, id) =>
                         i.Flag == 1 && i.Inuse == 0 && i.Status != 2 && id.BizRegID == tempBizRegID && (Convert.ToInt32(($"DATEDIFF(DAY, DATEADD(DAY, { interval }, { id.ExpiryDate }), GETDATE())")) >= 90))
                  .Execute();
                //.ToString();
                var dataOpen = _invoiceHDRRepo
                  .Count(i => new
                  {
                      TotalOpen = i.Status
                  })
                ...
                var dataOverAmount = _invoiceHDRRepo
                 .Sum(i => new
                 {
                     TotalOverdueAmount = i.TotalAmt1
                 })
                 .InnerJoin<DTO.Accounting.INVOICEITEM>(i => i.BatchCode, id => id.BatchCode, (i, id) => i.MonthCode == id.MonthCode)
                 .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((i, id) =>
                        i.Flag == 1 && i.Inuse == 0 && i.Status != 2 && id.BizRegID == tempBizRegID && (Convert.ToInt32(($"DATEDIFF(DAY, DATEADD(DAY, { interval }, { id.ExpiryDate }), GETDATE())")) >= 90))
                ...
                var dataInvoice = _invoiceHDRRepo
                            .Count(i => new { TotalInvoice = i.Status })
                            .InnerJoin<DTO.Accounting.INVOICEITEM>(i => i.BatchCode, id => id.BatchCode, (i, id) => i.MonthCode == id.MonthCode)
                            .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((i, id) =>
                                   i.Flag == 1 && i.Inuse == 0 && id.BizRegID == tempBizRegID)
                            .Execute();

                var dataAmountInvoice = _invoiceHDRRepo
                            .Sum(i => new { TotalAmount = i.TotalAmt1 })
                            .InnerJoin<DTO.Accounting.INVOICEITEM>(i => i.BatchCode, id => id.BatchCode, (i, id) => i.MonthCode == id.MonthCode)
                            .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((i, id) =>
                                   i.Flag == 1 && i.Inuse == 0 && i.Status != 2 && id.BizRegID == tempBizRegID)
                            .Execute();
                //OverduePercent = string.Format("CAST(CAST(TotalOverdue AS Decimal) / CAST(TotalInvoice AS DECIMAL) * 100 AS DECIMAL)"),

                return (dataOver, dataOverAmount, dataInvoice, dataAmountInvoice, dataOpen);
            }
            catch (Exception ex)
            {
                Log.Error(nameof(AccountingManager), ex);
        ...
        public async Task<dynamic> GetRecentInvoiceQ(Model.InvoiceFilter input)
        {
            try
            {
                //var invoiceQLogic = new Repo.INVOICEHDR_OLD(_env, _connectionString);
                input.MaxResultCount = 3;
                var tempBizRegID = input.customerCode;
                _serverExtAddress = _appConfiguration["AppExternal:ServerDomainAddress"];

                var data = _invoiceHDRRepo
                    .Select<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, id) => new
                    {
                        InvoiceNo = ih.BatchCode,
                        issuedDate = id.InvDate,
                        Amount = ih.TotalAmt1,
                    ...
                    .LeftJoin<DTO.Accounting.INVOICEITEM, DTO.Core.Entity.BIZENTITY>(id => id.BizRegID, be => be.BizRegID)
                    .Where<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, id) =>
                           ih.Flag == 1 && ih.Inuse == 0 && ih.Status != 2 && id.BizRegID == tempBizRegID)
                    .GroupBy<DTO.Questil.Payment.INVOICEHDR_CWMS, DTO.Accounting.INVOICEITEM>((ih, id) => new
                    {
                        InvoiceNo = ih.BatchCode,
                        issuedDate = id.InvDate,
                        Amount = ih.TotalAmt1,
                    ...
                    .OrderByDesc<DTO.Accounting.INVOICEITEM>(id => id.InvDate)
                    .Fetch(input.SkipCount > 0 ? input.SkipCount : 0, input.MaxResultCount > 0 ? input.MaxResultCount : 0)
                    .ExecuteList<Map.MapInvoiceQList>();
        ...
        public async Task<dynamic> GetSampleInvoice(int? num)
        {
            try
            {
                INVOICEITEM invoiceLogic = new INVOICEITEM(_env, _connectionString);
                int top = (num != null ? (int)num : 100);

                var data = invoiceLogic
                    .Select((i) => new { Participant = i.CompanyName, Price = i.Amount1, SubmissionTime = i.InvDate })
                    .Top(top)
                    .Where((i) => i.Flag == 1)
                    .GroupBy((i) => new { Participant = i.CompanyName, Price = i.Amount1, SubmissionTime = i.InvDate, i.CreateDate })
                    .OrderByDesc((i) => i.CreateDate)
                    .Execute();
