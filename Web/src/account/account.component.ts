...
export class AccountComponent extends AppComponentBase implements OnInit {
    ...
    startBgSlideShow(): void{
        let loginBoxWrp = document.querySelectorAll('#loginBoxWrp'),
        imgArry = ['login0.jpg','login1.jpg','login2.jpg'],
        indexArr = 0;
        setInterval(function(){
            if(indexArr == imgArry.length-1){
                indexArr = 0;
            }
            else indexArr++;
            loginBoxWrp[0].classList.add('animated');
            loginBoxWrp[0].classList.add('fadeIn');
            loginBoxWrp[0].setAttribute('style','background-image:url(assets/qstl/img/'+imgArry[indexArr]+')');
            }, 5000);
    }
