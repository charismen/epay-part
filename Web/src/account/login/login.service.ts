...
export class LoginService {
            ...
            if (provider.name === ExternalLoginProvider.FACEBOOK) {
                ...
                this.MSAL.loginPopup({
                    ...
                    this.MSAL.acquireTokenSilent({ scopes: scopes }).then((accessTokenResponse: AuthResponse) => {
                        ...
                        abp.log.error(error);
                        abp.message.error(this.localization.localize('CouldNotValidateExternalUser', this.localizationSourceName));
                    });
                });
            }
        });
    ...
    private login(accessToken: string, encryptedAccessToken: string, expireInSeconds: number, refreshToken: string, refreshTokenExpireInSeconds: number, rememberMe?: boolean, twoFactorRememberClientToken?: string, redirectUrl?: string, userData?: any): void {
